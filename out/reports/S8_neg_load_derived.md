...
- **运行 ID**：`run_20260920_104158_S8_neg_load_derived`
- **分析路线**：S8_neg_load_derived / 思路 B
- **耗时**：0.66 s
...
| 字段 | 值 |
|---|---|
| path | `F:\qcx\code\CrossLayersAnalyzer\out\samples\S8_neg_load_derived.bin` |
| sha256 | `122308ede5b4ac61c712a555238afd637046efceb4909f70dc04b93de497c6b2` |
| size | `20` |
| format | `bin` |
| arch | `riscv32` |
...
### 1. 存在但操作数不受外部输入影响（值静态未知）

这类情形**不是**「操作数为常量」，而是「攻击者影响不了它」——例如操作数来自常量地址的内存读取。结论同样是不可利用，但证据性质不同，不应混为一谈。

| 地址 | 关键指令 | 说明 |
|---|---|---|
| `0x80000000` | `divu arith_div` | 操作数不可由外部输入影响（无输入污点可达），但其具体值静态不可知（如从常量地址 load）。结论：攻击者无法影响该触发条件；注意这与「操作数是常量」不同 —— 只是「攻击者控制不了」。 |
...
```json
{
  "run_id": "run_20260920_104158_S8_neg_load_derived",
  "firmware_sha256": "122308ede5b4ac61c712a555238afd637046efceb4909f70dc04b93de497c6b2",
