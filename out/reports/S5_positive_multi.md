...
- **运行 ID**：`run_20260920_104156_S5_positive_multi`
- **分析路线**：S5_positive_multi / 思路 B
- **耗时**：0.99 s
...
| 字段 | 值 |
|---|---|
| path | `F:\qcx\code\CrossLayersAnalyzer\out\samples\S5_positive_multi.bin` |
| sha256 | `f99287da48205c3c48dc14c56a440882c4994fe6edc4ac5c48c7f126984d049d` |
| size | `40` |
| format | `bin` |
| arch | `riscv32` |
...
⚠️ **路径相关（path_dependent）**：该位置的触发条件在**部分路径**上成立、在另一部分路径上不成立。按路径分别评估，不应笼统归入可行或不可行。
...
以下位置的触发片段**存在且可达**，但操作数可控性取决于进入该基本块的路径，因此不能简单归入可行或不可行。

### 路径相关 1：`0x8000001c`

- 关键指令：`divu arith_div` @ `0x80000020`
- 到达路径长度：2 个基本块，入口 `bb_80000000`
- 污点源：（无外部输入污点）
- 约束进展度：50.00%
- 操作数可控性取决于进入本块的路径：在部分路径上为常量、在另一部分路径上可变。需按具体路径分别评估，不能笼统判为可控或不可控。
...
| 地址 | 关键指令 | 说明 |
|---|---|---|
| `0x8000001c` | `divu arith_div` | 操作数可控性取决于进入本块的路径：在部分路径上为常量、在另一部分路径上可变。需按具体路径分别评估，不能笼统判为可控或不可控。 |
...
```json
{
  "run_id": "run_20260920_104156_S5_positive_multi",
  "firmware_sha256": "f99287da48205c3c48dc14c56a440882c4994fe6edc4ac5c48c7f126984d049d",
